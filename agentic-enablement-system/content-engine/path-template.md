...
Copy this template, replace all `{{parameters}}` with values from the video spec (`video-spec-template.yaml`), and publish as `enablement/paths/RIU-XXX-topic-slug.md`.
...
**Topic**: {{topic}}
**What we're building**: {{output_description}}
**Concept from video**: {{concept_summary}}
**This path is part of**: {{constellation_name}} ({{constellation_sequence}})
...
2. **Confidence baseline**: "Before we begin — on a scale of 1-5, how confident are you right now that you could {{confidence_baseline_task}}? 1 = no idea where to start, 5 = could do it in my sleep."
...
**You'll produce**: {{quick_start_output}}

**Example to make it concrete**: {{quick_start_example}}

**Steps**:
{{quick_start_steps}}

**⚠ Common mistakes** (watch for these):
{{quick_start_failure_modes}}
...
1. **Real-world fit**: Does this solve a real problem from my domain, or is it still a practice exercise? If practice, push me to make it real.
2. **Standalone test**: Could someone on my team use this without me explaining it? If not, what's missing?
...
**You'll produce**: {{production_output}}
...
**✓ Portfolio review** — When I finish, ask me to share my output AND walk you through my design reasoning. Then evaluate:
1. **Technical soundness**: Does this follow known good patterns? Where does it diverge, and is that intentional?
2. **Tradeoff awareness**: What did I choose NOT to do? Can I articulate why? (If I can't explain the tradeoffs, I don't fully understand the design.)
3. **Failure modes**: What breaks first under stress? What's the recovery plan?
...
<!-- routing-targets: {{routing_targets}} -->
...
## FILLED EXAMPLE: Building a Taxonomy

A complete filled example showing what a published path looks like with all v2.1 features.
